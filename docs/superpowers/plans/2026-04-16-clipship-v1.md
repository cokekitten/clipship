...
```bash
pnpm create tauri-app clipship-app --template svelte-ts --manager pnpm
```
...
- [ ] **Step 2: Run test to verify it fails at compile**

Run: `cd src-tauri && cargo test config::`

Expected: compiles and tests PASS (we wrote test and implementation together here because the module is small and interlocked). If they don't pass, fix before moving on.
...
- [ ] **Step 1: Write the failing tests and the validators together**
...
mod tests {
    ...
    fn host_accepts_common_forms() {
        for ok in ["example.com", "10.0.0.1", "::1", "[::1]", "my_host"] {
...
Expected: all six tests (happy + 5 branches, each verifying its notification) PASS.
...
In `lib.rs` `setup`, after building state, load the config (if exists) and call `shortcut::register(&app.handle(), &cfg.shortcut)`. If no config exists yet, skip.
...
For macOS clipboard / notification / global-shortcut permissions, the current UX relies on the OS's own prompt the first time each capability is used. If the prompts are declined and the feature silently fails later, surface it through the same notification channel: `Message::ConfigInvalid` for clipboard read failures, and a dedicated log line for shortcut registration failures inside `shortcut::register`. No additional dialog is shown at startup in v1.
...
| Notifications and Errors (all 13 message variants, including platform-specific SshBinariesMissing hint) | Task 11, wired in Tasks 13/14/15/17/19/20/21 |
...
**Type consistency:**
- `Config` fields match across Rust + TypeScript bridge (`host`, `port`, `username`, `private_key_path`, `remote_dir`, `shortcut`, `version`).
